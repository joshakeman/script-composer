...
func (c *CharacterRepo) ListAll() []Character {
	...
	for rows.Next() {
		var ch Character
		err := rows.Scan(&ch.Name, &ch.ImagePath, &ch.ID)
		if err != nil {
			log.Fatal(err)
		}
		res = append(res, ch)
...
func (c *CharacterRepo) Create(name, imgPath string) error {
	stmt, err := c.db.Prepare(`INSERT INTO characters(name, img_path) VALUES(?, ?)`)
	if err != nil {
		log.Fatal(err)
	}

	res, err := stmt.Exec(name, imgPath)
	if err != nil {
		log.Fatal(err)
	}
